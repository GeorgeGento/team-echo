...
export default function ServerChatItem({
    ...
    const onMemberClick = () => {
        if (author?.id === currentMember?.id) return;
    ...
    const isOwner = currentMember?.id === author?.id;
            ...
            <div className="relative group flex items-center hover:bg-black/5 p-4 transition w-full">
                <div className="group flex gap-x-2 items-start w-full">
                    <div onClick={onMemberClick} className="cursor-pointer hover:drop-shadow-md transition">
                        <UserAvatar src={author?.user.imageUrl} />
                    </div>
                ...
                {canDeleteMessage && (
                    <div className="hidden group-hover:flex items-center gap-x-2 absolute p-1 top-1 right-6 bg-white dark:bg-zinc-800 border rounded-sm">
                        {canEditMessage && (
